...
const sort = (message) => {
  ...
  if (!values || values.length <= 5) {
    message.reply(`Você(s) tem ${values.length} jogador(es). É preciso no mínimo 6 para sortear um MIX.`)
  }
    ...
    const calculateLevel = (team) => Math.floor(team.reduce((acc, player) => acc + parseInt(`${player.lvl}`), 0) / team.length)
    ...
    message.reply(JSON.stringify(match)) 
}
export default sort